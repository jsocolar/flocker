...
#' Define the single-fp family
#' @param max_rep the maximum number of repeat sampling events at a unit
#' @return a "customfamily" "brmsfamily" object from brms
occupancy_single_fp <- function(max_rep) {
  brms::custom_family(
    "occupancy_single_fp", dpars = c("mu", "occ"),
    links = c("identity", "identity"),
    type = "real", 
...
#' Define the colonization-extinction-fp family
#' @param max_year the maximum number of seasons at a colex unit
#' @param max_rep the maximum number of repeat sampling events at a closure unit
#' @return a "customfamily" "brmsfamily" object from brms
occupancy_multi_colex_fp <- function(max_year, max_rep) {
  brms::custom_family(
    "occupancy_multi_colex_fp", dpars = c("mu", "occ", "colo", "ex"),
    links = c("identity", "identity", "identity", "identity"),
    type = "real", 
    # Integer aterms (vint) for n_unit, n_rep, rep_index1...
    vars = c(paste0("vint", seq(5 + max_year + max_rep))),
...
#' @param max_rep the maximum number of repeat sampling events at a closure unit
#' @return a "customfamily" "brmsfamily" object from brms
occupancy_multi_colex_eq_fp <- function(max_year, max_rep) {
  brms::custom_family(
    "occupancy_multi_colex_eq_fp", dpars = c("mu", "colo", "ex"),
    links = c("identity", "identity", "identity"),
    type = "real", 
    # Integer aterms (vint) for n_unit, n_rep, rep_index1...
    vars = c(paste0("vint", seq(5 + max_year + max_rep))),
