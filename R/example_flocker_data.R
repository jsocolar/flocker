...
example_flocker_data <- function(visit_constant = FALSE, seed = 123) {
  ...
  logit_psi <- 0 + 1*site_covs$sc1 - 1*site_covs$sc2 + stats::rnorm(20)[site_covs$grp]
  ...
  if (visit_constant) {
    ...
    logit_theta <- matrix(rep((-.5 + .5*site_covs$sc1 + stats::rnorm(20)[site_covs$grp]), 4), nrow = nsite) +
