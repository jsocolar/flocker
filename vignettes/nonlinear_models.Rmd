...
`brms`. In most occupancy models, occupancy and detection probabilties are 
...
more flexibile functions of covariates. These are straightforward to fit in
...
#> Intercept       -0.90      0.13    -1.16    -0.66 1.00     2294     2302
#> L_Intercept      0.50      0.10     0.37     0.75 1.00     1221      977
#> k_Intercept      0.19      0.08     0.07     0.37 1.00     1200     1719
#> t0_Intercept    -5.90      3.41   -10.41     3.03 1.00     1223     1078
#> dummy           -0.00      0.08    -0.15     0.15 1.00     2752     2526
#> 
...
The `gp()` function in `brms` includes a Gaussian process of arbitary dimension
...
And here's how we can fit this model in `flocker`! It turns out that we need
quite a lot of data points to constrain the standard deviation of the Gaussian
process, and so we use a Hilbert space approximate Gaussian process for
...
#>                Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> sdgp(g_gpxy)       1.69      0.78     0.73     3.67 1.00     3368     3076
#> lscale(g_gpxy)     0.23      0.11     0.08     0.50 1.00     4128     3128
#> 
#> Population-Level Effects: 
#>                  Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept           -1.04      0.06    -1.15    -0.93 1.00     6089     2975
#> occint_Intercept     0.09      0.09    -0.08     0.27 1.00     6221     3132
#> 
#> Draws were sampled using sampling(NUTS). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
