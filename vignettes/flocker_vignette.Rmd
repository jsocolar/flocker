---
title: 'Occupancy models with flocker: getting started'
...
To report bugs, request features, or contribute to `flocker`, check out [our page on GitHub](https://github.com/jsocolar/flocker/).

## Terms and defintions
The following terms feature importantly in this vignette. Some are not stanard in the literature (but we think maybe they should be):
...
**visit-constant**, **visit-varying**: We refer to models where $\theta$ is constant across visits within closure-units as *visit-constant models*. Occupancy models sometimes include covariates for detection that vary across visits to the same closure-unit. We call these models *visit-varying models*, as distinct from *visit-constant models*. It turns out that visit-constant models enable a more efficient parametrization of the likelihood than visit-varying models. It also turns out that this visit-constant likelihood enables some further bells and whistles, including within-chain parallelization of model fitting and better cross-validation, that are not available with the visit-varying likelihood.
...
Many occupancy models also include covariates that influence occupancy or detection probabilities. Sometimes covariates correspond to properties of a closure-unit (or a set of closure-units), such that they never vary across the repeated visits within a closure-unit. These *constant covariates* can be passed to `make_flocker_data()` as a dataframe with the same number of rows as the observation matrix and data in the same order as the rows of the observation matrix. Columns are covariates, and we recommend using informative column names. Alternatively, covariates (including visit-varying covariates) can be passed as a named list of matrices whose elements `[i, j]` are the covariate values for the visit represented by the corresponding position of the observation matrix. Again, we recommend using informative names for the list elements.
