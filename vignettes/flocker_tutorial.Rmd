...
mastering the formula syntax available in `brms`. 

This vignette introduces `flocker` with examples focused on "standard" single-season models (including multispecies models). For help with other model types, following the links here for [the data-augmented model](), [multiseason (dynamic) models](), and [false-positive models](). In the remainder of this vignette, we
...
Here's an example of how we format data, using example data provided with the package:
```{r data formatting}
library(flocker)
names(example_flocker_data)
names(example_flocker_data$event_covs)
head(example_flocker_data$obs) # observation matrix
head(example_flocker_data$unit_covs) # observation matrix
head(example_flocker_data$event_covs$ec1)
flocker_data <- make_flocker_data(
  obs = example_flocker_data$obs,
  unit_covs = example_flocker_data$unit_covs,
  event_covs = example_flocker_data$event_covs
  )
...
### Flexible linear predictors: Gaussian processes and splines
Via `brms`, `flocker` supports Gaussian processes (`brms::gp()`) as well as `mgcv` syntax for thin-plate regression splines (`brms::s()`) and tensor product smooths (`brms::t2()`). For example:
```{r mgcv, eval=FALSE}
flock(f_occ = ~ s(uc1), 
      f_det = ~ 1, 
      flocker_data = flocker_data)
...
`brms` is capable of fitting a variety of additional effects structures. The following structures should translate directly to `flocker`, but we have yet to test them with occupancy models specifically.
...
`get_Z()` accepts several additional arguments that control the way that posterior is obtained and the values returned. See `?get_Z` for details.
