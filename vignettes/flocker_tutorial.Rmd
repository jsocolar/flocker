...
```{r lme4_2, eval=FALSE}
mymod <- flock(f_occ = ~ uc1 + (1 + uc1 | species), 
      f_det = ~ ec1 + (1 | species), 
      flocker_data = flocker_data)
```
...
```{r lme4_3, eval=FALSE}
mymod <- flock(f_occ = ~ uc1 + (1 |g1| species) + (0 + uc1 |g2| species), 
...
## Post-processing
COMING SOON: this is implemented for rep-constant models and partially implemented for rep-varying models.  It should be fully implemented, tested, and documented very soon.