...
This vignette is intended as a companion to Socolar & Mills 2023 (INSERT LINK),
...
for several types of model, from data simulation, through model fitting, 
to model post-processing. We also showcase the `brms` syntax 
that `flocker` can use to fit a variety of section showcasing some of the 
flexibility that `flocker` incorporates from `brms` to fit sophisticated effect 
structures.
...
* **closure-unit**: The groupings of observations over which
 ...
 (multiseason) models, a closure-unit is a site-season combination (or 
 species-site-season in a multispecies dynamic model).
...
library(flocker)
```
to make `flocker` functions globally available within an R session.
...
```{r simulate}
d_rep_varying <- simulate_flocker_data()
```

The simulated data `d_rep_varying` are in list form, with elements for the 
detection/non-detection observations `d_rep_varying$obs`, unit covariates 
`d_rep_varying$unit_covs`, and event covariates `d_rep_varying$event_covs`. 
`d_rep_varying$obs` is a matrix where rows are species-site combinations, 
columns are replicate visits, and entries are `1` (detection), `0` 
(nondetection), or `NA` (no visit). `d_rep_varying$unit_covs` is a dataframe 
...
expects a highly specific data format. The function `make_flocker_data()` 
...
detection probabilities. Unit covariates (see *Terms and definitions* above) can 
...
column names. *Event covariates* (see *Terms and definitions* above) can be 
...
`get_Z()` provides the posterior distribution for the 
...
occupancy model likelihood that we explain in more detail in the companion paper
INSERT LINK and [here](https://jsocolar.github.io/likelihoodOccupancy/).
...
probability that a given (pseudo)species occurrs in the metacommunity) are
available via `fitted_flocker`. For example:

```{r fitted-flocker, eval = FALSE}
...
`get_Z()` accepts several additional arguments that control the way that posterior is obtained and the values returned. See the companion paper INSERT LINK and 
`?get_Z` for details.

### Posterior prediction
The funtion `predict_flocker()` provides posterior predictions. By default, 
...
that posterior is obtained and the values of returned. See the companion paper 
INSERT LINK and `?predict_flocker` for details.
...
(see the companion paper INSERT LINK and 
[here](https://jsocolar.github.io/likelihoodOccupancy/) for details of why). 
...
then there will be two entries for each of the shared parameters in the prior 
table (`uc1` in this example). Specifying a priors for parameters in formulas 
...
internally center the design matrix, which affects the prior on the intercept 
...
brms::prior_summary(rep_constant_prior1)
```
...
whose parameters have their own covariate-based linear predictors. For example, 
suppose an expanding population of a territorial species undergoes logistic 
growth, and also that some territories are unsuitable due to an unobserved 
factor, such that occupancy asymptotes at some unknown probability less than 
one. Thus, occupancy probability changes through time as 
$\frac{L}{1 + e^{-k(t-t_0)}}$, where $L$ is the asymptote, $k$ is a growth rate, 
$t$ is time, and $t_0$ is the timing of the inflection point. On the logit 
scale, the occupancy probability changes as 
$\log(\frac{L}{1 + e^{-k(t - t_0)} - L})$. We can fit this model with:
...
fd_nonlinear <- make_flocker_data(
...
likely want to fit separate spatial terms by species \cite{Doser_spOccupancy}. 
...
Finally, we note that modelers whose primary interest is in fitting spatial 
terms might be well-served to investigate the functionality of R package
`spOccupancy` INSERT LINK, which affords some spatial specifications not yet 
available via `brms` (e.g spatially varying coefficients). On the other hand,
`brms` (and therefore `flocker`) likewise provides multiple spatial structures 
not available in `spOccupancy`, and `flocker` enables these structures to 
combine with the full flexibility of the additional effects structures provided 
by `brms`.
