...
`get_Z` function:
```{r Z, echo = FALSE, eval = FALSE}
Z <- get_Z(multi_colex_eq)

```

We can generate posterior predictions:
```{r predict, echo = FALSE, eval = FALSE}
pp <- predic
```

We can compare the fit of these various models using approxiamte leave-one-out
cross-validation, where the holdouts consist of entire series 
(leave-one-series-out cross-validation).

```{r loo, echo = FALSE, eval = FALSE}
