...
To extend to the Bernoulli misclassification (bmc) model, we first introduce a random number generator that samples from the sampling distribution of the number of binomial trials `n` that resulted in an observed number of successes `s` given success probability $\rho$ and a flat prior.
```{r rng, eval = FALSE}
flocker:::extended_binomial_rng
...
omc_generator_single2 <- function(
  ...
  fd_null <- simulate_flocker_data(
    n_pt = N, n_sp = 1, fp = NULL, 
    params = list(
      det_intercept = rnorm(1, -1, .5),
  ...
  vf <- attributes(fd_null)$visit_full
  
  expected_real_ones_prop <- 
    sum(boot::inv.logit(vf$occ) * boot::inv.logit(vf$logit_det)) /
    (4 * N)
  expected_real_zeros_prop <- 1 - expected_real_ones_prop
  expected_false_ones_prop <- (expected_real_ones_prop / rho) - expected_real_ones_prop
#  p_false_one_given_real_zero <- expected_false_ones_prop / expected_real_zeros_prop
  p_false_one_given_real_zero <- 0.1
  assertthat::assert_that(
    p_false_one_given_real_zero < 1,
    msg = "there are not enough real zeros to accommodate the false ones!"
    )
  
  fd <- fd_null
  for(i in seq_len(nrow(fd$obs))){
    for(j in seq_len(ncol(fd$obs))){
      if((fd$obs[i,j] == 1) | (runif(1) < p_false_one_given_real_zero)){
        fd$obs[i,j] <- rho
      }
    }
  }
  L1_given_0 <- p_false_one_given_real_zero
  L0_given_0 <- 1 - L1_given_0
  fop <- flocker:::new_matrix(
    fd$obs, 
    (fd$obs == 0) * L0_given_0 + 
...
functions {
  ...
  // Initialize and compute log-likelihood
    ...
    for (i in 1:vint1[1]) {
      array[vint2[i]] int indices = index_array[i, 1:vint2[i]];
      if (vint3[i] == 1) { // if there is a one with certainty
