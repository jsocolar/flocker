...
In the "colonization-extinction" model, logit colonization probabilities 
...
functionality to handle $\mathcal{O}$ differently. Any pair of 
colonization/extinction probabilities, if they remain constant through time, 
defines an equilibrium proportion of sites that are occupied. If colonization 
and extinction remain constant for a long time prior to the first timestep, the 
initial occupancy probability will simply be this equilibrium frequency.  
...
This can sometimes be valid even if the probabilities are time-varying over the 
series being modeled, for example if a some sites are subject to a disturbance 
that perturbs equilbrium dynamics during the timeseries. For a concrete
example, consider a study system that is initially in equilibrium, but in which
some sites burn in a forest fire partway through the study. If the impact of 
the burn on colonization and extinction probabilities is modeled adequately, 
...
```{r colex-explicit, echo = FALSE, eval = FALSE}
multi_colex <- flock(
  f_occ = ~ uc1,
  f_det = ~ uc1 + ec1,
  f_col = ~ uc1,
...
```{r colex-equilibrium, echo = FALSE, eval = FALSE}
...
e.g. `f_auto = ~ uc1`.
```{r auto-explicit, echo = FALSE, eval = FALSE}
multi_auto <- flock(
  f_occ = ~ uc1,
  f_det = ~ uc1 + ec1,
  f_col = ~ uc1,
...
the first timestep:
```{r auto-equilirium, echo = FALSE, eval = FALSE}
multi_auto_eq <- flock(
  f_det = ~ uc1 + ec1,
  f_col = ~ uc1,
  f_auto = ~ 1,
