...
is assumed to apply within (but not across) timesteps, but the latent occupancy
...
distributions for the latent occupancy state.] The HMM conceptualizes the 
latent occupancy state as a Markov-like process (it need not be strictly 
Markovian, because the transition probabilities can vary by timestep), where the 
transition probabilities between the two possible states (occupied or 
unoccupied) are a colonization probabilility $\mathcal{C}$, an extinction 
probability $\mathcal{E}$, and their complements $1 - \mathcal{C}$ and 
$1 - \mathcal{E}$. 

To specify the HMM, we need a model for the $\mathcal{C}$, a model for 
$\mathcal{E}$, a model for the initial occupancy probability $\mathcal{O}$ in 
the first timestep, and a model for the probability of observing the data in 
any particular timestep conditional on the true occupancy state in that 
timestep (in the HMM literature, these probabilities are referred to as 
...
sub-model. In `flocker`, we assume that the detection probabilities vary 
logit-linearly with a suite of covariates that can vary across all levels of
...
In the "colonization-extinction" model, colonization probabilities $\mathcal{C}$
and extinction probabilities $\mathcal{E}$ are both assumed to vary 
logit-linearly according to separate linear predictors, whose component 
covariates may vary across timesteps within series but never at the visit level
...
colonization probability and the autologistic parameter gives the probability of
persistence $\mathcal{P}$; i.e. the complement of the extinction probability 
...
In addition to the standard autologistic model that models the offset as a
constant, `flocker` also affords the flexibility to model the offset via its
own linear predictor, which reintroduces flexibility for the relationship
between $\mathcal{P}$ and $\mathcal{C}$ to vary, but tends to retain the idea
that these probabilities ought to be related to one another and ought to tend
to move in tandem.

#### Initial occupancy
Most examples of multiseason occupancy models in the literature include an
explict model for the initial occupancy probability $\mathcal{O}$ via its own
linear predictor, which is analogous to the linear predictor in for occupancy
in a single-season model. Thus, a colonization-extinction model would contain
linear predictors for detection, colonization, extinction, and initial 
...
and initial occupancy. `flocker` is well equipped to fit these models.

Borrowing from the HMM literature, `flocker` also comes with functionality to
handle $\mathcal{O}$ a bit differently. Any set of colonization/extinction 
probabilities, if they remain constant through time, defines an equilibrium
proportion of sites that are occupied. Thus, if colonization and extinction
probabilities can be assumed to have remained constant for a long time prior
to the first timestep, the initial occupancy probability will simply be this 
equilibrium frequency.  `flocker` is equipped to fit models without any separate
linear predictor for initial occupancy, and instead to begin with the
equilibrium frequencies implied by the coloniziation and extinction 
probabilities modeled for the first timestep. This can sometimes be valid even 
if the probabilities are time-varying over the series being modeled, for
example if a disturbance perturbs equilbrium dynamics during the timeseries. It
is for this reason that, when using equilibrium frequencies as intial occupancy
probabilities, `flocker` always computes the equilibrium based on the 
probabilites modeled for the first timestep.

As an aside, it is not uncommon in the literature to see confusion that
conflates the initial occupancy probabilities $\mathcal{O}$ with the
colonization probabilities $\mathcal{C}$. This confusion arises from the
notion that the autologistic model consists of a single-season occupancy model
plus an autologistic term, which leads people to incorrectly surmise that the
linear predictor without the autologistic term gives an occupancy 
probability.^[Well, maybe initial occupancy probabilities and colonization 
probabilities could be equivalent in a study of species introduction or of 
primary succession, but otherwise no!] 
...
in the first timestep. Depending on hardware, fitting this model might take 2-5 
minutes.

```{r colex-explicit, echo = FALSE}
multi_colex <- flock(
  f_occ = ~ uc1,
  f_det = ~ uc1 + ec1,
  f_col = ~ uc1,
...
```{r colex-equilibrium, echo = FALSE}
