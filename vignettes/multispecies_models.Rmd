...
$1 - \mathcal{E}$. The complement of the extinction probability turns out to be
particularly important; we call it the *persistence* probability 
...
logit-linearly according to separate linear predictors based on  
covariates that may vary across timesteps within series but never across
visits within closure-units.
...
own linear predictor, which reintroduces flexibility in the relationship
between $\mathcal{P}$ and $\mathcal{C}$, but tends to retain the idea
...
functionality to handle $\mathcal{O}$ differently. Any pair of colonization/extinction 
probabilities, if they remain constant through time, defines an equilibrium
proportion of sites that are occupied. If colonization and extinction remain
constant for a long time prior to the first timestep, the initial occupancy 
probability will simply be this equilibrium frequency.  `flocker` is equipped to 
fit models without any separate linear predictor for initial occupancy, and 
instead to begin with the equilibrium frequencies implied by the coloniziation 
and extinction probabilities modeled for the first timestep. This can sometimes 
be valid even if the probabilities are time-varying over the series being 
modeled, for example if a disturbance perturbs equilbrium dynamics during the 
timeseries. It is for this reason that, when using equilibrium frequencies as 
initial occupancy probabilities, `flocker` always computes the equilibrium based 
on the probabilites modeled for the first timestep.
